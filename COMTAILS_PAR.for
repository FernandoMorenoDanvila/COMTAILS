...
C -----------------------------------------------------------------------
C We are very grateful to Man-To Hui for bringing to our attention
C a typographical error in Sekanina and Larson (1984) eq. (7), that
C we have correctly implemented in this version (routine ANISOT_DIR2)
      
C --- COMTAILS mpi parallel version -----
...
C MPI:
         ...
         INTEGER START_TIME, END_TIME, START_SIZE, END_SIZE
         INTEGER START_EVENT, END_EVENT
         REAL*8 TIME_ARR(NTIMEMAX),TIME_ARRMPI(NTIMEMAX)
         REAL*8 RC_ARR(NTIMEMAX),RC_ARRMPI(NTIMEMAX)
         REAL*8 DMDT_ARR(NTIMEMAX),DMDT_ARRMPI(NTIMEMAX)
         REAL*8 SUBSOLAR_ARR(NTIMEMAX),SUBSOLAR_ARRMPI(NTIMEMAX)
         REAL*8 THETAC_ARRMPI(NTIMEMAX)
C MISCELLANEOUS VARIABLES         
         ...
         REAL*8 TOTAL_MASS,TOTAL_MASSMPI,SFWHM,LAT,LON,COSZ,EXPOCOS
         REAL*8 RSUNKM,SOLID_ANGLE,ARCSEC_RAD,AREAPIX,CTE_PART,CTE_NUC
         REAL*8 RHO_AP,MAG,AFRHO,AFRHO_0,PCP
         INTEGER NX,NY,NSIZES,NTIMES,ITIME,IRLOG,IAP,NTIMES_IN
...
C VARIABLES TO BUILDUP DUST TAIL IMAGE/STAR FIELD IMAGE        
         ...
         REAL*4 FLUXTIME(NXMAX,NYMAX),FLUXMPI(NXMAX,NYMAX)
         REAL*4 CONVOLU(NXMAX,NYMAX),OPT_DEPTH(NXMAX,NYMAX)
         REAL*4 OPT_DEPTHTIME(NXMAX,NYMAX),OPT_DEPTHMPI(NXMAX,NYMAX)
...
C ROTATING NUCLEUS AND ACTIVE AREA PARAMETERS
         REAL*8 NUC_PHI,NUC_INC,SINI,COSI,THETA0_T,CL1,CL2
...
C INITIALIZE ARRAY OF FLUXES AND OPTICAL DEPTH:
         DO II=1,NX
            DO JJ=1,NY
               ...
               FLUXMPI(II,JJ)=0
               OPT_DEPTH(II,JJ)=0
               OPT_DEPTHTIME(II,JJ)=0
               OPT_DEPTHMPI(II,JJ)=0
               FLUX_STAR(II,JJ)=0
            ENDDO
         ENDDO
         OPT_DEPTH_NUC=0
...
C TIME ARRAY         
     ...
     &    PER_JD,RA,DEC,DELTA,PSANG,PLANG,PHAS_ANG)
        ...
        CALL MPI_BCAST(XARR,3*NTIMEMAX,MPI_REAL8,0,
     &             MPI_COMM_WORLD,IERR)
        CALL MPI_BCAST(VARR,3*NTIMEMAX,MPI_REAL8,0,
     &             MPI_COMM_WORLD,IERR)
        CALL MPI_BCAST(XYZ_EARTH,3,MPI_REAL8,0,
     &             MPI_COMM_WORLD,IERR)
        CALL MPI_BCAST(QR,1,MPI_REAL8,0,
     &             MPI_COMM_WORLD,IERR)
        CALL MPI_BCAST(EC,1,MPI_REAL8,0,
     &             MPI_COMM_WORLD,IERR)
        CALL MPI_BCAST(INC,1,MPI_REAL8,0,
     &             MPI_COMM_WORLD,IERR)
        CALL MPI_BCAST(OM,1,MPI_REAL8,0,
     &             MPI_COMM_WORLD,IERR)
        CALL MPI_BCAST(WPER,1,MPI_REAL8,0,
     &             MPI_COMM_WORLD,IERR)
        CALL MPI_BCAST(TRUEANARR,NTIMEMAX,MPI_REAL8,0,
     &            MPI_COMM_WORLD,IERR)
       CALL MPI_BCAST(PER_JD,1,MPI_REAL8,0,
     &             MPI_COMM_WORLD,IERR)
       CALL MPI_BCAST(RA,1,MPI_REAL8,0,
     &             MPI_COMM_WORLD,IERR)
       CALL MPI_BCAST(DEC,1,MPI_REAL8,0,
     &             MPI_COMM_WORLD,IERR)
       CALL MPI_BCAST(DELTA,1,MPI_REAL8,0,
     &             MPI_COMM_WORLD,IERR)
       CALL MPI_BCAST(PSANG,1,MPI_REAL8,0,
     &             MPI_COMM_WORLD,IERR)
       CALL MPI_BCAST(PLANG,1,MPI_REAL8,0,
     &             MPI_COMM_WORLD,IERR)
       CALL MPI_BCAST(PHAS_ANG,1,MPI_REAL8,0,
     &             MPI_COMM_WORLD,IERR)

    
        CALL MPI_BCAST(XX,1,MPI_REAL8,0,
     &             MPI_COMM_WORLD,IERR)
        CALL MPI_BCAST(XY,1,MPI_REAL8,0,
     &             MPI_COMM_WORLD,IERR)
        CALL MPI_BCAST(XZ,1,MPI_REAL8,0,
     &             MPI_COMM_WORLD,IERR)
        CALL MPI_BCAST(YX,1,MPI_REAL8,0,
     &             MPI_COMM_WORLD,IERR)
        CALL MPI_BCAST(YY,1,MPI_REAL8,0,
     &             MPI_COMM_WORLD,IERR)
        CALL MPI_BCAST(YZ,1,MPI_REAL8,0,
     &             MPI_COMM_WORLD,IERR)
        CALL MPI_BCAST(ZX,1,MPI_REAL8,0,
     &             MPI_COMM_WORLD,IERR)
        CALL MPI_BCAST(ZY,1,MPI_REAL8,0,
     &             MPI_COMM_WORLD,IERR)
        CALL MPI_BCAST(ZZ,1,MPI_REAL8,0,
     &             MPI_COMM_WORLD,IERR)
     ...
     &  WRITE (*,9) START_JD,START_JD-PER_JD,END_JD,END_JD-PER_JD
         ...
         IF (EC.LT.1.D0) THEN                   ! elliptic orbit
            ...
            IF (RANK.EQ.0) WRITE (*,10) ORB_PER/365.25D0
         ENDIF
...
C COMET HELIOCENTRIC DISTANCE IN DATE-OBS in au:         
        ...
        IF (RANK.EQ.0) WRITE (*,17) RCOBS,DELTA
...
c        ENDIF ! ALL PROCESSES WITH RANK=0
        
        
         IF (RANK.EQ.0) WRITE (*,*) ' Building up dust tail ...'
        
C ----------------------------------------
C -------------- TIME LOOP ---------------
C ----------------------------------------
C TIME INTERVALS FOR PARALLEL CALCULATION ACCORDING TO NPROCS: 
        start_time = rank * (NTIMES-1) / size + 1
        end_time = (rank + 1) * (NTIMES-1) / size

        TOTAL_MASS=0.0D0

c         DO 7776 ITIME=1,NTIMES-1
         DO 7776 ITIME=start_time,end_time

C INITIALIZE FLUX AND OPTICAL DEPTH ARRAYS FOR TIME INTERVAL ITIME
        DO II=1,NX
	DO JJ=1,NY
           FLUXTIME(II,JJ)=0.
           OPT_DEPTHTIME(II,JJ)=0
...
C          
          IF (IEJEC_MODE.EQ.3) THEN
             ...
             SUBSOLAR_ARR(ITIME)=SUBSOLAT/TORAD
C            WRITE (29,24) TIME-PER_JD,RC_EJEC,THETAC_EJEC,SUBSOLAT/TORAD
          ENDIF        
...
c        write (*,*) AREA_LATITUDE/TORAD,AREA_LONGITUDE/TORAD
     ...
     &       THETA0_T,SINI,COSI,UR,UTHETA,UZ)
      ...
      IF (NSHADOW.EQ.NEVENT) THEN ! All the area is in shadow
         IF (RANK.EQ.0) WRITE (*,990) TIME-PER_JD
...
C HAVING LPAR > 0
 ...
 7775  TOTAL_MASS=TOTAL_MASS+DMDT*DELTAT*86400.D0 ! TOTAL DUST MASS EJECTED
       ...
       TIME_ARR(ITIME)=TIME-PER_JD
       RC_ARR(ITIME)=RC_EJEC
       DMDT_ARR(ITIME)=DMDT
       
C       WRITE (77,995) TIME-PER_JD,RC_EJEC,DMDT ! SERIAL

C ADDUP FLUXES AND OPT. DEPTH IN CORRESPONDING ARRAYS FOR THIS TIME INTERVAL
	   DO II=1,NX
	      DO JJ=1,NY
                 FLUX(II,JJ)=FLUX(II,JJ)+FLUXTIME(II,JJ)
                 OPT_DEPTH(II,JJ)=OPT_DEPTH(II,JJ)+OPT_DEPTHTIME(II,JJ)
             ENDDO
          ENDDO
       
 7776  ENDDO    ! Time sampling

         CALL MPI_REDUCE(TIME_ARR,TIME_ARRMPI,NTIMES,MPI_REAL8,
     &      MPI_SUM, 0,MPI_COMM_WORLD, ierr)

         CALL MPI_REDUCE(RC_ARR,RC_ARRMPI,NTIMES,MPI_REAL8,
     &      MPI_SUM, 0,MPI_COMM_WORLD, ierr)

         CALL MPI_REDUCE(DMDT_ARR,DMDT_ARRMPI,NTIMES,MPI_REAL8,
     &        MPI_SUM, 0,MPI_COMM_WORLD, ierr)
         
          CALL MPI_REDUCE(SUBSOLAR_ARR,SUBSOLAR_ARRMPI,NTIMES,MPI_REAL8,
     &        MPI_SUM, 0,MPI_COMM_WORLD, ierr)
               
          CALL MPI_REDUCE(TRUEANARR,THETAC_ARRMPI,NTIMES,MPI_REAL8,
     &        MPI_SUM, 0,MPI_COMM_WORLD, ierr)
               
         CALL MPI_REDUCE(TOTAL_MASS,TOTAL_MASSMPI,1,MPI_REAL8,
     &      MPI_SUM, 0,MPI_COMM_WORLD, ierr)

       
       IF (RANK.EQ.0) THEN
       WRITE (*,18) TOTAL_MASSMPI
       DO ITIME=1,NTIMES-1
       WRITE (77,995) TIME_ARRMPI(ITIME),RC_ARRMPI(ITIME),
     &         DMDT_ARRMPI(ITIME)
       WRITE (29,24) TIME_ARRMPI(ITIME),RC_ARRMPI(ITIME),
     &       THETAC_ARRMPI(ITIME),SUBSOLAR_ARRMPI(ITIME)      
       ENDDO
       ENDIF

       
         CALL MPI_REDUCE(FLUX,FLUXMPI, NXMAX*NYMAX, MPI_REAL,
     &      MPI_SUM, 0,MPI_COMM_WORLD, ierr)

         CALL MPI_REDUCE(OPT_DEPTH,OPT_DEPTHMPI, NXMAX*NYMAX, MPI_REAL,
     &      MPI_SUM, 0,MPI_COMM_WORLD, ierr)
       
C ADD NUCLEUS CONTRIBUTION TO IMAGE, ATTENUATED BY OPTICAL THICKNESS:
          
           FLUXMPI(INUC,JNUC)=FLUXMPI(INUC,JNUC)+SNGL(BRNUCLEUS*
     &      EXP(-OPT_DEPTH_NUC))
c            IF (RANK.EQ.0) write (*,*)' NUC ',FLUXMPI(INUC,JNUC)
...
C -- ONLY WORKS FOR HIGHLY SAMPLED IMAGES, i.e. LARGE NTIMES x NSIZES x NEVENT)
          DO II=1,NX
           DO JJ=1,NY
            FLUX_STAR(II,JJ)=FLUX_STAR(II,JJ)*EXP(-OPT_DEPTHMPI(II,JJ))
           ENDDO
         ENDDO
...
C BLOCKED, BUT THIS VERY UNLIKELY CASE IS NOT ACCOUNTED FOR  
           DO II=1,NX
              DO JJ=1,NY
               FLUXMPI(II,JJ)=FLUXMPI(II,JJ)+FLUX_STAR(II,JJ)
            ENDDO
         ENDDO
...
C -----------------------------------
         ...
         IF (ICONV.EQ.1) THEN              
         CALL CONVOLY(SNGL(SFWHM),FLUXMPI,CONVOLU,NX,NY)
         DO II=1,NX
         DO JJ=1,NY
         FLUXMPI(II,JJ)=CONVOLU(II,JJ)
         ENDDO
         ENDDO
         ENDIF

C OUTPUT SYNTHETIC TAIL IMAGE IN SOLAR DISK INTENSITY UNITS (FITS FORMAT):      
         CALL WRITEIMAGE (OUTIMAGE,RECID,FLUXMPI,NX,NY)
C COMPUTES AFRHO AND MAGNITUDE FOR SELECTED APERTURE AROUND THE PHOTOCENTER
     ...
     &        GRDSIZ,PHAS_ANG,RHO_AP,AFRHO,AFRHO_0,MAG)
         IF (RANK.EQ.0) WRITE (*,23) RHO_AP,AFRHO,MAG
...
C OUTPUT SYNTHETIC TAIL IMAGE IN MAG/ARCSEC**2 (FITS FORMAT):         
         ...
         DO II=1,NX
         DO JJ=1,NY
         IF (FLUXMPI(II,JJ).GT.0.0) THEN      
         FLUXMPI(II,JJ)=SNGL(CTE_MAG)+MAGSUN-2.5*LOG10(FLUXMPI(II,JJ))
         ELSE
         FLUXMPI(II,JJ)=100.  ! mag/arcsec**2 (i.e., ~zero flux)
         ENDIF   
         ENDDO
         ENDDO
...
C OPTICAL DEPTH IMAGE (FITS FORMAT):      
       ...
       CALL WRITEIMAGE (OUTIMAGE,RECID,OPT_DEPTHMPI,NX,NY)      
...
C ---END OF PROCEDURE ---------------        
         ...
         IF (RANK.EQ.0) WRITE (*,21) (TIME_END-TIME_START)/60.
C -------------------------------------------------         
 8       FORMAT (1X,
...
C m_star= CTE_MAG + MAGSUN -2.5*log10(i/i_0) (mag/arcsec**2)
	...
	DO I=1,NX
	   DO J=1,NY
C              IF (IMAGE(I,J).NE.0.0) WRITE (*,*) IMAGE(I,J) !!!!!
...
C       THETA0_T=DATAN(DTAN(NUC_PHI)*COSI)
c Original Sekanina & Larson (1984) equation (7):
c       THETA=(TWOPI/PERIOD)*(TIME-PER_JD)+THETA0_T+THETA0-AREA_LONGITUDE
c     Corrected for typos (thanks to Man-To Hui):
